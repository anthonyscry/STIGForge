...
<#
    ...
    .INPUTS
        ...
        -ExcludeVuln <String>
        Specify which Group ID(s) to exclude from the scan (V-#####).  For multiple, separate with commas.  Can only be used with -SelectSTIG.
...
Process {
    Try {
        ...
        $EvaluateStigVersion = "1.2510.0"
        $ReleaseDate = "2025-Dec-11"
        ...
        if ($SelectInstance) {
            $SelectInstance = $SelectInstance -split ","
        }
        if ($ExcludeInstance) {
            $ExcludeInstance = $ExcludeInstance -split ","
        }
        if ($SelectDatabase) {
            $SelectDatabase = $SelectDatabase -split ","
        }
        if ($ExcludeDatabase) {
            $ExcludeDatabase = $ExcludeDatabase -split ","
        }
        if ($SelectSite) {
            $SelectSite = $SelectSite -split ","
        }
        if ($ExcludeSite) {
            $ExcludeSite = $ExcludeSite -split ","
        }
        ...
        # If both -SelectInstance and -ExcludeInstance are specified, error out.  Only one may be specified.
        if (($SelectInstance) -and ($ExcludeInstance)) {
            $ExitCode = 1
            throw "-SelectInstance and -ExcludeInstance cannot both be specified."
        }

        # If both -SelectDatabase and -ExcludeDatabase are specified, error out.  Only one may be specified.
        if (($SelectDatabase) -and ($ExcludeDatabase)) {
            $ExitCode = 1
            throw "-SelectDatabase and -ExcludeDatabase cannot both be specified."
        }

        # If both -SelectSite and -ExcludeSite are specified, error out.  Only one may be specified.
        if (($SelectSite) -and ($ExcludeSite)) {
            $ExitCode = 1
            throw "-SelectSite and -ExcludeSite cannot both be specified."
        }
        # ---------- End Prechecks ----------
        ...
        If ($Version) {
            ...
            foreach ($URL in $UpstreamRootURLs) {
                try {
                    $UpstreamFileList =[XML]($WebClient.DownloadString("$($URL)/Src/Evaluate-STIG/xml/FileList.xml"))
                    $UpstreamVersion = $UpstreamFileList.FileList.Version
                    $UpstreamReleaseDate = $UpstreamFileList.FileList.ReleaseDate
                    break
                }
                catch {
                    # Do nothing
            ...
            if ($UpstreamVersion) {
                if ($UpstreamVersion -gt [Version]$EvaluateStigVersion) {
                    Write-Host " Evaluate-STIG $($UpstreamVersion) is available.  Please upgrade. " -BackgroundColor DarkCyan -ForegroundColor White
                }
            }
            else {
                $UpstreamVersion = "Unable to obtain version from upstream"
            }
            if (-not($UpstreamReleaseDate)) {
                $UpstreamReleaseDate = "Unable to obtain release date from upstream"
            }

            $Result = [PSCustomObject]@{
                LocalVersion       = $EvaluateStigVersion
                LocalVersionDate   = $ReleaseDate
                RunningPath        = $PSScriptRoot
                CurrentVersion     = $UpstreamVersion
                CurrentVersionDate = $UpstreamReleaseDate
            }
            Return $Result
        }
        ...
        If ($ComputerName) {
            ...
            if ($SelectDatabase) {
                $HashArguments.Add("SelectDatabase", $SelectDatabase)
            }
            if ($ExcludeDatabase) {
                $HashArguments.Add("ExcludeDatabase", $ExcludeDatabase)
            }
            if ($SelectInstance) {
                $HashArguments.Add("SelectInstance", $SelectInstance)
            }
            if ($ExcludeInstance) {
                $HashArguments.Add("ExcludeInstance", $ExcludeInstance)
            }
            if ($SelectSite) {
                $HashArguments.Add("SelectSite", (Receive-NormalizedArray -Array $SelectSite -Action Quoted))
            }
            if ($ExcludeSite) {
                $HashArguments.Add("ExcludeSite", (Receive-NormalizedArray -Array $ExcludeSite -Action Quoted))
            }
            If ($AltCredential) {
        ...
        ElseIf ($ESXiApplianceName -or $VCenterApplianceName) {
            ...
            # Write utilized preference settings to log
                            ...
                            Try {
                                ...
                                ForEach ($instance in $ApacheInstances) {
                                    ...
                                    ForEach ($vhost in $instance.VirtualHosts) {
                                        ...
                                        $AddJob = $true
                                        # Check -SelectSite|-ExcludeSite before adding job
                                        if (($SelectSite -and $ScanArgs.ModuleArgs.AnswerFileVars.Site -notin $SelectSite) -or ($ExcludeSite -and $ScanArgs.ModuleArgs.AnswerFileVars.Site -in $ExcludeSite)) {
                                            # Site is either not part of -SelectSite or is excluded with -ExcludeSite
                                            $AddJob = $false
                                            if ($SelectSite -and $ScanArgs.ModuleArgs.AnswerFileVars.Site -notin $SelectSite) {
                                                Write-Log -Path $STIGLog -Message "Ignoring job '$($BaseFileName)' per -SelectSite parameter" -Component $LogComponent -Type "Warning" -OSPlatform $OSPlatform
                                            }
                                            elseif ($ExcludeSite -and $ScanArgs.ModuleArgs.AnswerFileVars.Site -in $ExcludeSite) {
                                                Write-Log -Path $STIGLog -Message "Ignoring job '$($BaseFileName)' per -ExcludeSite parameter" -Component $LogComponent -Type "Warning" -OSPlatform $OSPlatform
                                            }
                                        }
                        ...
                        "JBoss" {
                            Try {
                                ...
                                ForEach ($instance in $JBossInstances) {
                                    ...
                                    $AddJob = $true
                                    # Check -SelectInstance|-ExcludeInstance before adding job
                                    if (($SelectInstance -and $ScanArgs.ModuleArgs.AnswerFileVars.Instance -notin $SelectInstance) -or ($ExcludeInstance -and $ScanArgs.ModuleArgs.AnswerFileVars.Instance -in $ExcludeInstance)) {
                                        # Site is either not part of -SelectInstance or is excluded with -ExcludeInstance
                                        $AddJob = $false
                                        if ($SelectInstance -and $ScanArgs.ModuleArgs.AnswerFileVars.Instance -notin $SelectInstance) {
                                            Write-Log -Path $STIGLog -Message "Ignoring job '$($BaseFileName)' per -SelectInstance parameter" -Component $LogComponent -Type "Warning" -OSPlatform $OSPlatform
                                        }
                                        elseif ($ExcludeInstance -and $ScanArgs.ModuleArgs.AnswerFileVars.Instance -in $ExcludeInstance) {
                                            Write-Log -Path $STIGLog -Message "Ignoring job '$($BaseFileName)' per -ExcludeInstance parameter" -Component $LogComponent -Type "Warning" -OSPlatform $OSPlatform
                                        }
                                    }
                                ...
                                ForEach ($Instance in $allInstances) {
                                    If ($Instance.Status -eq "Running") {
                                        ...
                                        ForEach ($Database in $allDatabases) {
                                            ...
                                            # Check -SelectDatabase|-ExcludeDatabase before adding job
                                            if (($SelectDatabase -and $ScanArgs.ModuleArgs.AnswerFileVars.Database -notin $SelectDatabase) -or ($ExcludeDatabase -and $ScanArgs.ModuleArgs.AnswerFileVars.Database -in $ExcludeDatabase)) {
                                                # Site is either not part of -SelectDatabase or is excluded with -ExcludeDatabase
                                                $AddJob = $false
                                                if ($SelectDatabase -and $ScanArgs.ModuleArgs.AnswerFileVars.Database -notin $SelectDatabase) {
                                                    Write-Log -Path $STIGLog -Message "Ignoring job '$($BaseFileName)' per -SelectDatabase parameter" -Component $LogComponent -Type "Warning" -OSPlatform $OSPlatform
                                                }
                                                elseif ($ExcludeDatabase -and $ScanArgs.ModuleArgs.AnswerFileVars.Database -in $ExcludeDatabase) {
                                                    Write-Log -Path $STIGLog -Message "Ignoring job '$($BaseFileName)' per -ExcludeDatabase parameter" -Component $LogComponent -Type "Warning" -OSPlatform $OSPlatform
                                                }
                                            }
                        ...
                        "MongoDB3" {
                            Try {
                                ...
                                ForEach ($instance in $MongoInstances) {
                                    ...
                                    # Check -SelectDatabase|-ExcludeDatabase before adding job
                                    if (($SelectDatabase -and $ScanArgs.ModuleArgs.AnswerFileVars.Database -notin $SelectDatabase) -or ($ExcludeDatabase -and $ScanArgs.ModuleArgs.AnswerFileVars.Database -in $ExcludeDatabase)) {
                                        # Site is either not part of -SelectDatabase or is excluded with -ExcludeInstance
                                        $AddJob = $false
                                        if ($SelectDatabase -and $ScanArgs.ModuleArgs.AnswerFileVars.Database -notin $SelectDatabase) {
                                            Write-Log -Path $STIGLog -Message "Ignoring job '$($BaseFileName)' per -SelectDatabase parameter" -Component $LogComponent -Type "Warning" -OSPlatform $OSPlatform
                                        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDLKFNH2wrM27sw
# mUwXi3DWYka9VPQ3Mv8tjyx6aTIGAKCCHiQwggUqMIIEEqADAgECAgMTYdUwDQYJ
...
# BCDW68G8gKWVNWNsPIttLfOmy0hav6iibRngxSBNSnUTrTANBgkqhkiG9w0BAQEF
# AASCAQBe/NtzcsAKCsVLt5RbPB7ys6HUKPQ28NpqnE8s412vtg/IUlVM7eqGq1K3
# nd54lJpRo+Y+WhN5FWyszwrxSiRsJcpr1LB7vPpDjxNGgz6aK7BrpOY9hmqWXIut
# Lxtss8o+XEt/KrEJ3miCNVLgiNnR68xvydokGzgNy8BiJ6RBdDBpW14J1RW8EmPu
# sKZsBK16gkU/wWdKafTg2cDrsD/2uT9agoXq3zuniPoaDlg9X8GE4Io3u34c5TGg
# LBURLUXVpuBUxCEteJ+TSkvwIPND24CtbLgIgh8EUv7J1J8Kv/NJvF8SzAb0B+Fa
# j3n0a7heG20QdZmTltx0A3ayzAhJoYIDJjCCAyIGCSqGSIb3DQEJBjGCAxMwggMP
# AgEBMH0waTELMAkGA1UEBhMCVVMxFzAVBgNVBAoTDkRpZ2lDZXJ0LCBJbmMuMUEw
# PwYDVQQDEzhEaWdpQ2VydCBUcnVzdGVkIEc0IFRpbWVTdGFtcGluZyBSU0E0MDk2
# IFNIQTI1NiAyMDI1IENBMQIQCoDvGEuN8QWC0cR2p5V0aDANBglghkgBZQMEAgEF
# AKBpMBgGCSqGSIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI1
# MTIxMTIwMTQwNlowLwYJKoZIhvcNAQkEMSIEIA821P07RF3HxwAgg2chbvmSQM0k
# HSYgCJOBzbbid0LdMA0GCSqGSIb3DQEBAQUABIICAB6KXV/ZnHU0xJkgNGVGWxsc
# e/7/StRMWsls2wSa5LYVMOzOimiF4q95kPd8bs5gtzLHcv8QdknxBnbFHmqlTg89
# GAMzPcuCfPDZRSoWywLPrdlmVMG7GlMkiMHvU1nqGrqQYeMUx8mcH3pJZUQ+n4vF
# LjcUTGsMfGIcdU0MRSdqoaZZOcKbaeIz4Yjje/WP/3g5Hq6fHXD1HJ8/ImZ0F9sU
# NuI+yz+6KnFBhC4LbCzPv+8wlQVL0nbTPtoZlIk31iNE31W6YZOQoAqgpMIwciEd
# UxH8pT7P3KuXn88Kn30ZV/r33swU5kcpJYyrX+d1n0wGYo3/7D3+E1gg5DNfbp3k
# xLi2Ecc//thszWLBUSyIz5mUSEJeA1bpabwKz/ysnv36e3U3U7ntUBVD/Pfx2aIn
# DE9f2m02JID8oFAWyTLC4CgZcD4JsZtVGDQxSkhftEJiyr3mYgLHNtQNAMPKzzYP
# jbzec644MOuBHnEBrFeCZfbeDQ4FkizaACuyvxKBRfNwYy9BTgN+6VOkZExgiNzA
# rWPW/xk/xrrHakUdfWbicy5JFOsMadIG38wqcKvC0SjwSiSrevWat8G+FzoIFxK8
# 8Ai1U/u7XW6sQFNHFRi7uZJ32nR6Z0gzb+sqsDGkiC+C52DWXCjpecPdps0BzpnC
# Ws0g9fwwKTUl2kMLjqgd
