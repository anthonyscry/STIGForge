...
$PSDefaultParameterValues['out-string:width'] = 500
...
Function Get-V213929 {
    ...
    if (!$res) {
        ...
        $FindingDetails += "`nThe following triggers exist on the instances:`n$(
            $($res | Format-Table | Out-String | Remove-TrailingSpaces) -replace "`n","`n  " -replace "`n  $"
...
Function Get-V213930 {
    ...
    If ($res.'Authentication Mode' -eq 'Windows Authentication') {
        $Status = "NotAFinding"
        $FindingDetails = "Authention mode is Windows Authentication.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    Else {
        ...
        "
        $FindingDetails = "Authention mode is Windows Mixed. Verify the existing SQL accounts are documented and approved by the ISSO/ISSM.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)`n$($ress | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213931 {
    ...
    #---=== Begin Custom Code ===---#
    # Several changes made by Ken Row on 11/26/25:
    # - issue 2273 Corrected a problem in identifying the virtual account name on a default instance.
    # - issue 2274 Changed to properly handle default instances.
    # - issue 2271 Changed to prevent SQL Launchpad's service account from consideration.
    ...
    if ($darr.partofdomain) {
        ...
        "
        $sAcct = $res.acct
        if ($res.instance -like '*\*') {
            # This is a named instance
            $sHost = $res.instance -replace '\\.*$'
            $sInst = $res.instance -replace '^.*\\'
            $sVirtualAcct = "NT SERVICE\MSSQL`$$sInst"
        } else {
            # This is a default instance
            $sHost = $res.instance
            $sInst = $null
            $sVirtualAcct = "NT SERVICE\MSSQLSERVER"
        }
        $sFQDN = "$sHost.$sDom"

        $fVirtualAcct = ($sAcct -eq $sVirtualAcct)
        if ($fVirtualAcct) {
            $sAcct = "$sDom\$sHost`$"
        }
        ...
        if ($sInst) {
            $sPortlessSPN = "MSSQLSvc/${sFQDN}:${sInst}"
        } else {
            $sPortlessSPN = "MSSQLSvc/${sFQDN}"
        }

        $aTargetSPNs = @{
          $sPortlessSPN=$false
        ; "MSSQLSvc/${sFQDN}:$iPort"=$false
        }
        ...
        if ($arrSPN.count -gt 1) {
            ...
            foreach ($i in 1..$($arrSPN.count - 1)) {
                ...
                if ($aTargetSPNs.ContainsKey($sSPN)) {
                    $aTargetSPNs[$sSPN]=$true
                } else {
                    $sExcess += "  $sSPN`n" # we found one we did not expect
                }
            } # foreach ($i in 1..$($arrSPN.count - 1))

            if ($aTargetSPNs.ContainsValue($true)) {
                $fGood = $true
                $FindingDetails += "The following required SPNs for ${sAcct} exist as expected:`n`n"
                $aTargetSPNs.GetEnumerator() | where-object Value -eq $true | foreach-object {
                    $FindingDetails += "  $($_.Name)`n"
                }
                $FindingDetails += "`n"
            }
            ...
            if ($sExcess -gt '') {
                $fGood = $false
                $FindingDetails += "The following unexpected SPNs were found for account ${sAcct}:`n`n$sExcess`n`n"
            }

            if ($aTargetSPNs.ContainsValue($false)) {
                $fGood = $false
                $FindingDetails += "The following required SPNs for ${sAcct} were not found:`n`n"
                $aTargetSPNs.GetEnumerator() | where-object Value -eq $false | foreach-object {
                    $FindingDetails += "  $($_.Name)`n"
                }
                $FindingDetails += "`n"
            }
            ...
            if ($fGood) {
                $status = "NotAFinding"
                $FindingDetails = "`nSetting to Not a Finding -- all required SPNs exist and no extra SPNs exist."
...
Function Get-V213932 {
    ...
    $($res  | Format-Table | Out-String | Remove-TrailingSpaces)
    $($res2 | Format-Table | Out-String | Remove-TrailingSpaces)"
...
Function Get-V213933 {
    ...
    " | Format-Table | Out-String | Remove-TrailingSpaces
...
Function Get-V213934 {
    ...
    If ($res) {
        $Status = "Open"
        $FindingDetails = "The following privileges need revoked from NT AUTHORITY\SYSTEM:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    Else {
        ...
        "
        $Status = "NotAFinding"
        $FindingDetails = "The correct permissions are assigned to NT AUTHORITY\SYSTEM.`n$($Stat | Format-Table | Out-String | Remove-TrailingSpaces)`n$($SYSPERMS | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213935 {
    ...
    If ($res) {
        ...
        try {
            ...
            If ($res3) {
                $Status = "Open"
                $FindingDetails = "One or more computer accounts were found, this is a finding:`n$($res3.path | Format-Table | Out-String | Remove-TrailingSpaces)"
            }
            else {
                $Status = "NotAFinding"
                $FindingDetails = "One or more accounts ending in `$ were found, however they are not computer accounts, this is not a finding:`n$($res.name | Format-Table | Out-String | Remove-TrailingSpaces)"
            }
        } catch {
            $Status = "Not_Reviewed"
            $FindingDetails = "Review the following list. If any are computer accounts, this is a finding.`n$($res3.path | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
    }
    Else {
        $Status = "NotAFinding"
...
Function Get-V213936 {
    ...
    if ($FindingDetails -eq "") {
        $Status = "NotAFinding"
        $FindingDetails = "All required events are being audited.`n$($InstalledAudits | Format-Table | Out-String | Remove-TrailingSpaces)`n$($AuditActions | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = 'Open'
    }
...
Function Get-V213937 {
    ...
    if ($res) {
        #$Status = 'Open'
        $FindingDetails += "DBA, ensure the following have been authorized by the ISSM to create and/or maintain audit definitions:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213939 {
    ...
    If ($res) {
        ...
        If ($res) {
            $Status = 'NotAFinding'
            $FindingDetails += "The audit is being performed.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
        Else {
...
Function Get-V213940 {
    ...
    if ($res) {
        ...
        $FindingDetails += "The check query found that audits start automatically.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "Open"
...
Function Get-V213941 {
    ...
    If (!$CheckAudits) {
        $Status = "NotAFinding"
        $FindingDetails = "All STIG audits are in use.`n$($InstalledAudits | Format-Table | Out-String | Remove-TrailingSpaces)`n$($AuditActions | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    Else {
        $FindingDetails = "Verify audits are in use and match your system documentation.`n$($InstalledAudits | Format-Table | Out-String | Remove-TrailingSpaces)`n$($AuditActions | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213942 {
    ...
    if ($res) {
        $Status = 'NotAFinding'
        $FindingDetails += "The check query found that SQL Sever will shut down upon audit failure.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
...
Function Get-V213948 {
    ...
    "
    If ($res) {
        $FindingDetails += "Accounts with audit-related permissions:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    Else {
        $Status = "NotAFinding"
        $FindingDetails = "No results were returned by the check queries."
...
Function Get-V213954 {
    ...
    if ($res) {
        $Status = "Open"
        $FindingDetails = "The following demonstration/sample databases should not exist on a production server:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "NotAFinding"
...
Function Get-V213955 {
    ...
    Else {
        $Status = "Not_Reviewed"
        $FindingDetails += "Microsoft SQL Product Features Installed:`n"
        $FindingDetails += $res | Format-Table | Out-String | Remove-TrailingSpaces
...
Function Get-V213957 {
    ...
    Get-ISQL -ServerInstance $Instance -Database $Database $qry | ForEach-Object {
        ...
        $FindingDetails += "Instance $($_.InstanceName) is $sState with xp_cmdshell enabled.`n$($_ | Format-Table | Out-String | Remove-TrailingSpaces)"
    } # foreach-object
...
Function Get-V213958 {
    ...
    Get-ISQL -ServerInstance $Instance -Database $Database $qry | ForEach-Object {
        ...
        $FindingDetails += "Instance $($_.InstanceName) is $sState with clr enabled.`n$($_ | Format-Table | Out-String | Remove-TrailingSpaces)"
    } # foreach-object
...
Function Get-V213959 {
    ...
    if ($res) {
        $Status = 'Open'
        $FindingDetails += "DBA, ensure the SSP documents the following Non-Standard extended stored procedures as required:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "NotAFinding"
...
Function Get-V213960 {
    ...
    if ($links) {
        $FindingDetails = "Check the system documentation to see if the following linked servers are required and approved:`n$($links | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    ...
    "
    if ($res) {
        $FindingDetails += "A linked server is defined with a remote name, which potentially allows sysadmin impersonation:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213961 {
    ...
    $FindingDetails += $((Get-Variable regprotocol* -ErrorAction SilentlyContinue | Format-Table | Out-String | Remove-TrailingSpaces) -replace 'regprotocol', '' -replace 'Name *Value', 'Protocol      Enabled' -replace '--*  *--*', ' ')
...
Function Get-V213963 {
    ...
    $FindingDetails = "Verify all listed accounts and members of security groups are not shared accounts.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
...
Function Get-V213964 {
    ...
    If ($res.'Authentication Mode' -eq 'Windows Authentication') {
        ...
        "
        $FindingDetails += $($ress | Format-Table | Out-String | Remove-TrailingSpaces)
...
Function Get-V213965 {
    ...
    If ($ress) {
        ForEach ($res in $ress) {
            ...
            If ($res2) {
                $Status = 'Open'
                $FindingDetails += "This contained database has users using SQL authentication:`n$($res2 | Format-Table | Out-String | Remove-TrailingSpaces)"
            }
        }
        If ($FindingDetails -eq '') {
            $FindingDetails += "DBA, ensure the following contained databases are documented as authorized:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
    }
    Else {
        $Status = "NotAFinding"
...
Function Get-V213966 {
    ...
    If ($res.'Authentication Mode' -eq 'Windows Authentication') {
        $Status = 'Not_Applicable'
        $FindingDetails = "Set to Not Applicable because authention mode is Windows Authentication.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    Else {
...
Function Get-V213967 {
    ...
    if ($FindingDetails -eq '') {
        ...
        $Status = 'Open'
        $FindingDetails = "The TLS and SSL settings are Not in compliance.`n$($FindingDetails | Format-Table | Out-String | Remove-TrailingSpaces)" # JJS Added
    }
...
Function Get-V213970 {
    ...
    if (!$res) {
        ...
        $Status = "Not_Reviewed"
        $FindingDetails = "Verify server documentation to ensure accounts are documented and unique:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
ORDER BY [Database Name]"
    if (!$res) {
        ...
        $Status = "Not_Reviewed"
        $FindingDetails = "For each user database where encryption is required, verify that encryption is in effect:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213975 {
    ...
    if ($SQLEdition.edition -match $EditionReg) {
        $Status = "Open"
        $FindingDetails = "Common Criteria Compliance is only available in Enterprise and Data Center Editions of SQL Server. Per DISA STIG Support, any SQL Edition that does not support CCC is a permanent 'Open'.`n$($SQLEdition | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $res = Get-ISQL -ServerInstance $Instance -Database $Database "
        SELECT @@servername as instance
        ...
        if ($res.value_in_use -ne 1) {
            $Status = 'Open'
            $FindingDetails = "Instance does not have Common Criteria Compliance enabled. If disabling CCC has been documented and approved due to performance reasons, then this may be downgraded to a CAT III finding.`n$($SQLEdition | Format-Table | Out-String | Remove-TrailingSpaces). `n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
        Else {
            $Status = 'NotAFinding'
            $FindingDetails = "Instance has Common Criteria Compliance enabled.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
    }
...
Function Get-V213976 {
    ...
    if ($res) {
        $FindingDetails += "DBA, confirm that IFI is documented as required for this account:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213978 {
    ...
    if ($res) {
        $FindingDetails = "Review user list to make sure SQL Server reveals detailed error messages only to the ISSO, ISSM, SA, and DBA:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
    ...
    if ($res -and $res.Status -eq 1) {
      $FindingDetails += "Checked and verified that error details are omitted from user error messages via trace flag 3625:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
...
Function Get-V213979 {
    ...
    $FindingDetails += "" | Out-String
    $FindingDetails += $res  | Format-Table | Out-String | Remove-TrailingSpaces
    $FindingDetails += $res2 | Format-Table | Out-String | Remove-TrailingSpaces
...
Function Get-V213980 {
    ...
    if ($res) {
        #$Status = 'Open'
        $FindingDetails += "DBA, ensure the following have been documented as authorized for use by external processes:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "NotAFinding"
        $FindingDetails = "No results were returned by the check queries."
...
Function Get-V213983 {
    ...
    Else {
        If ($res.storage_type -in 'APPLICATION LOG', 'SECURITY LOG' -and $res.storage_type -notin 'FILE') {
            $Status = "Not_Applicable"
            $FindingDetails = "Audit logs are using APPLICATION or SECURITY event logs rather than writing to FILE, this is Not Applicable.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
    }
    ...
    If ($res -and $Status -ne "Not_Applicable") {
        ...
        "
        If ($res2) {
            $FindingDetails = "The following log files and settings were examined:`n$($res2 | Format-Table | Out-String | Remove-TrailingSpaces)"
            $fOK = $true
            $arrBad = @()
        ...
        $res | ForEach-Object {
            if ($_.DefaultDomain -eq 'NULL') {
                ...
                if ($ts -eq 'Local CMOS Clock') {
                    $FindingDetails += "Instance $($_.instance) does not appear to sync with a time server.`n$($_ | Format-Table | Out-String | Remove-TrailingSpaces)"
                }
            }
        } # $res | foreach-object
        if ($FindingDetails -eq '') {
...
Function Get-V213987 {
    ...
    "
    if ($res) {
        $FindingDetails += "DBA, ensure the following have been documented as authorized to control the server:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    ...
    if ($res) {
        $FindingDetails += "DBA, ensure the following have been documented as authorized to administer the server:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213988 {
    ...
    if ($res) {
        $FindingDetails += "DBA, ensure the following have been documented as authorized to be in the server's local Administrators group:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "NotAFinding"
        $FindingDetails = "No results were returned by the check queries."
...
Function Get-V213989 {
    ...
    if ($res) {
        ...
        if ($res2) {
            $Status = 'Open'
            $FindingDetails += "The following actions are not being audited:`n$($res2 | Format-Table | Out-String | Remove-TrailingSpaces)"
        } # if ($res2)
    }
    else {
        $Status = 'Open'
...
Function Get-V213990 {
    ...
    "
    if ($res) {
        $FindingDetails += "DBA, If the following protocols are not documented as required and authorized, they must be disabled:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    ...
    if ($res) {
        $FindingDetails += "The following services are not authorized and should be disabled:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213994 {
    ...
    if (!$res) {
        ...
        $Status = "Not_Reviewed"
        $FindingDetails = "Verify Security-relevant software updates to SQL Server must be installed within the time period directed by an authoritative source (e.g. IAVM, CTOs, DTMs, and STIGs):`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213995 {
    ...
    "
    If ($res) {
        $FindingDetails += "An audit is configured and running:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    Else {
        $FindingDetails += "No audit is configured or running.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        $Compliant = $False
    }
    ...
    "
    If ($res) {
        $FindingDetails += "`nThe SCHEMA_OBJECT_ACCESS_GROUP is included in the server audit specification.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    Else {
        $Compliant = $False
        $FindingDetails = "`nThe SCHEMA_OBJECT_ACCESS_GROUP was not returned in an active audit.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V214004 {
    ...
    if ($res) {
        ...
        if ($res) {
            $Status = 'NotAFinding'
            $FindingDetails = "The SCHEMA_OBJECT_CHANGE_GROUP audit is being performed.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
...
Function Get-V214006 {
    ...
    If ($res) {
        ...
        If ($res) {
            $Status = 'NotAFinding'
            $FindingDetails += "The SCHEMA_OBJECT_ACCESS_GROUP audit is being performed.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
        Else {
...
Function Get-V214014 {
    ...
    if ($res) {
        ...
        if ($res) {
            ...
            if ($res2) {
                $Status = 'NotAFinding'
                $FindingDetails = "The FAILED_LOGIN_GROUP and SUCCESSFUL_LOGIN_GROUP audit are being performed.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
            } else {
                $Status = "Open"
...
Function Get-V214018 {
    ...
    if ($res) {
        $Status = 'NotAFinding'
        $FindingDetails += "The SUCCESSFUL_LOGIN_GROUP audit is being performed.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
...
Function Get-V214026 {
    ...
    if ($res) {
        $Status = 'Open'
        $FindingDetails += "Has CEIP participation been documented as authorized on this system?  The following registry settings were found:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "NotAFinding"
        $FindingDetails = "No results were returned by the check queries."
...
Function Get-V214027 {
    ...
    $res = Get-Service sqltelemetry* | Where-Object StartType -NE 'Disabled'
    if ($res) {
        $FindingDetails += "Telemetry is enabled on this system, and probably should not be. If it is authorized, further STIG checks are needed of telemetry auditing.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "NotAFinding"
...
Function Get-V214028 {
    ...
    if ($res) {
        if ($res.is_disabled -ne $true) {
            $FindingDetails += "The SQL Server default account [$($res.name)] account is not disabled on the instance.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
    }
    else {
...
Function Get-V214029 {
    ...
    if ($res) {
        if ($res.name -eq 'sa') {
            $FindingDetails += "The SQL Server default account has not been renamed.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
    }
...
Function Get-V214030 {
    ...
    if ($res) {
        $FindingDetails += "Ensure the following stored procedures have been documented as authorized to run at startup:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "NotAFinding"
        $FindingDetails = "No results were returned by the check query."
...
Function Get-V214031 {
    ...
    if ($res) {
        $Status = 'Open'
        $FindingDetails += "The following should either be encrypted or documented as authorized for unencrypted transmission:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "NotAFinding"
        $FindingDetails = "No results were returned by the check query."
...
Function Get-V214033 {
    ...
    if ($res) {
        $Status = 'Open'
        $FindingDetails += "Has the accessing of the registry via extended stored procedures been documented as requried and authorized?:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "NotAFinding"
        $FindingDetails = "No results were returned by the check query."
...
Function Get-V214041 {
    ...
    if ($FindingDetails -gt ' ') {
        $FindingDetails = "External Scripts Enabled is enabled.`n$($FindingDetails | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "NotAFinding"
        $FindingDetails = "External Scripts Enabled is not enabled.`n$($FindingDetails | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V214042 {
    ...
    if ($res) {
        if ($res.StartType -eq 'Disabled') {
            $Status = "NotAFinding"
            $FindingDetails = "The SQL Browser is disabled.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
        else {
            $FindingDetails = "The SQL Browser service is not disabled, but if it has been documented and approved as required, this is not a finding.`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
    }
    else {
...
Function Get-V214043 {
    ...
    if ($FindingDetails -gt ' ') {
        $FindingDetails = "Replication Xps is enabled.`n$($FindingDetails | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "NotAFinding"
...
Function Get-V214044 {
    ...
    if ($FindingDetails -eq '') {
        ...
        $FindingDetails = "The system's instances and sql browser are not hidden and/or configured properly.`n$($FindingDetails | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V265870 {
    ...
    $FindingDetails = "The following SQL-related products are installed: $($productlist | Format-Table | Out-String | Remove-TrailingSpaces)The current version of SQL itself is: $sqlver"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBUmOF1sz84Rihv
# E/G65SuYFaFR2FUQQDENSFEOnfSfh6CCHiQwggUqMIIEEqADAgECAgMTYdUwDQYJ
...
# BCA3K731mFOwyIWSau8nscvpTs57/ChEPaUv1ZerKB7RyTANBgkqhkiG9w0BAQEF
# AASCAQCEggXkLtUKPzODs918a/KiFQp3zygVSFON68xMnKPTA8Hf1oC7o5ZPVY4f
# ArtqkUftXDqzRMcttDJl+B908vxfxxK7lnngzY/23CxWMPyq0WHks+eLCjt17PEh
# oSTZHj5Pw7uArbkXYiAYtLIWjHUCjPbam/lYcDiN5Nkba0nlSIwiN0x158ZY9nge
# IpZwG3OIEVZllLWAuZUJmWNff8GhGOrLNew3An5G86dksg/+Ho+ECqq1vj1bxiFs
# aE59P57TeSDKT0d+MZDoey9vqaJkDZ7YmeInB3GyXoPmxs6S8gSPpUjp3GqR5dIb
# 9morb0S21kK/QHUS98+gJRWV1FaWoYIDJjCCAyIGCSqGSIb3DQEJBjGCAxMwggMP
# AgEBMH0waTELMAkGA1UEBhMCVVMxFzAVBgNVBAoTDkRpZ2lDZXJ0LCBJbmMuMUEw
# PwYDVQQDEzhEaWdpQ2VydCBUcnVzdGVkIEc0IFRpbWVTdGFtcGluZyBSU0E0MDk2
# IFNIQTI1NiAyMDI1IENBMQIQCoDvGEuN8QWC0cR2p5V0aDANBglghkgBZQMEAgEF
# AKBpMBgGCSqGSIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI1
# MTIxMTE4NTIxNlowLwYJKoZIhvcNAQkEMSIEICxurPydGgcaZYjBgJYavD8kbOjH
# XPYp3BrJIve2Z9GCMA0GCSqGSIb3DQEBAQUABIICAFI8VwPypaGgZa7+1nGpLtms
# 8aetvgq69/2X1pS8J756Tus1SN3cmciT/9eyvEJmvZs7blNPzZIrfLGiZG2R2q2d
# sBxu3GEdZEoPWjzzl5j+5cGnViPVpwLvMlYtgn9jasFYx/GstzD1+kRlY8gOxJ/F
# 5maBCupfcGjmVqGJbX1Nn4RAlUx1mIWwi3HXg2Jq634MNfg+9WflwWryf9VEdFbJ
# Uez6qEpxeip0i2VpioiL61r3PbvZWB0RyYU/SA0tUhfl3UsQQXKyxmWB7LPWuRyC
# ATuDx8CHVTDcHjPOcMIiY6+6t4F8qkYkDfxvTrzQ401U8VBIpk0V60J/4ix4fma5
# 2KoQUXbab4Z874+NQc3LsXLsF44qrABsiorCty0RV0+FpmlLlMKj2SinM9phXbrR
# wFDl22h6pdBYmHG/7CzloDr8tTjO8u5JfYCrjwKuTw+XC/JKxZ1N0BAEmwLJIxx/
# joySvFHKVCJTEyOoBHs7ihUM3rXMhRoLR1eKkh9SOy69CasVIlj3LRhQWLvl+NMV
# coUk5jPwmqWxLUL7jsrldobGyq4RN7i6vv5ssewwtG3uoFjgT/wBFSUL+SgmYfgC
# vtNgP0I448OgiOBwv6GhxlXPrt3Gq+zaiK15nuzZFex3O39G/SNvUuGonbvz0kBb
# E3zKc3lfu4VAmV6toIYf
