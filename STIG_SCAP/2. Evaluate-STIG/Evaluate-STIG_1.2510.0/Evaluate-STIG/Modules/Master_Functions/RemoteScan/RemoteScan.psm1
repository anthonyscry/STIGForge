    ...
    Try {
        ...
        $RemoteScriptBlock = {
            ...
            Try {
                ...
                if ($SelectDatabase) {
                    $ESArgs += "-SelectDatabase $((Receive-NormalizedArray -Array $SelectDatabase -Action Quoted) -join ',') "
                }
                elseif ($ExcludeDatabase) {
                    $ESArgs += "-ExcludeDatabase $((Receive-NormalizedArray -Array $ExcludeDatabase -Action Quoted) -join ',') "
                }

                if ($SelectInstance) {
                    $ESArgs += "-SelectInstance $((Receive-NormalizedArray -Array $SelectInstance -Action Quoted) -join ',') "
                }
                elseif ($ExcludeInstance) {
                    $ESArgs += "-ExcludeInstance $((Receive-NormalizedArray -Array $ExcludeInstance -Action Quoted) -join ',') "
                }

                if ($SelectSite) {
                    $ESArgs += "-SelectSite $((Receive-NormalizedArray -Array $SelectSite -Action Quoted) -join ',') "
                }
                elseif ($ExcludeSite) {
                    $ESArgs += "-ExcludeSite $((Receive-NormalizedArray -Array $ExcludeSite -Action Quoted) -join ',') "
                }
        ...
        if ($SelectDatabase) {
            $Params.SelectDatabase = $SelectDatabase
        }
        else {
            $Params.SelectDatabase = $False
        }

        if ($ExcludeDatabase) {
            $Params.ExcludeDatabase = $ExcludeDatabase
        }
        else {
            $Params.ExcludeDatabase = $False
        }

        if ($SelectInstance) {
            $Params.SelectInstance = $SelectInstance
        }
        else {
            $Params.SelectInstance = $False
        }

        if ($ExcludeInstance) {
            $Params.ExcludeInstance = $ExcludeInstance
        }
        else {
            $Params.ExcludeInstance = $False
        }

        if ($SelectSite) {
            $Params.SelectSite = $SelectSite
        }
        else {
            $Params.SelectSite = $False
        }

        if ($ExcludeSite) {
            $Params.ExcludeSite = $ExcludeSite
        }
        else {
            $Params.ExcludeSite = $False
        }
            ...
            Foreach ($LinuxHost in $LinuxList) {
                ...
                If ($PowerShellVersion -ge [Version]"7.1") {
                    Try {
                        ...
                        if ($SelectDatabase) {
                            $ESArgs += "-SelectDatabase $($SelectDatabase -join ',') "
                        }
                        elseif ($ExcludeDatabase) {
                            $ESArgs += "-ExcludeDatabase $($ExcludeDatabase -join ',') "
                        }

                        if ($SelectInstance) {
                            $ESArgs += "-SelectInstance $($SelectInstance -join ',') "
                        }
                        elseif ($ExcludeInstance) {
                            $ESArgs += "-ExcludeInstance $($ExcludeInstance -join ',') "
                        }

                        if ($SelectSite) {
                            $ESArgs += "-SelectSite $($SelectSite -join ',') "
                        }
                        elseif ($ExcludeSite) {
                            $ESArgs += "-ExcludeSite $($ExcludeSite -join ',') "
                        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC