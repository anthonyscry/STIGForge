...
Function Get-V213900 {
    ...
    if ($res.run_value -eq 0 -and $res.config_value -eq 0) {
        ...
        if ($res.AuthenticationMode -eq 'Windows Authentication') {
            ...
            "
            if ($res) {
                $FindingDetails += "Also, ensure the following accounts are documented as authorized to be managed by SQL Server:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
            }
        }
    }
...
Here are the row counts and checksums for the three queries in the supplemental STIG file 'Database permission assignments to users and roles.sql':`n$($resCount | Format-Table | Out-String | Remove-TrailingSpaces)
Details for the Database Owner query:`n$($resOwner | Format-Table | Out-String | Remove-TrailingSpaces)
Details for the Database Roles query:`n$($resRoles | Format-Table | Out-String | Remove-TrailingSpaces)
Details for the Privileges query:    `n$($resPrivs | Format-Table | Out-String | Remove-TrailingSpaces)
"
    } # If ($Database -eq "tempdb")
    #---=== End Custom Code ===---#
...
Function Get-V213902 {
    ...
    if ($res) {
        ...
        $FindingDetails = "The following accounts were found in SQL and checked against AD:`n$($outarray | Format-Table -HideTableHeaders | Out-String | Remove-TrailingSpaces)"
        if ($fUnknown) {
...
Function Get-V213903 {
    ...
    if ($res) {
        $FindingDetails += "If any tables listed in the documentation are not in this list, this is a finding.:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    } else {
...
Function Get-V213904 {
    ...
    if (!$res) {
        ...
        $FindingDetails = "DBA, Confirm that an approved server documentation documents the need for TRUSTWORTHY in the following:$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    $null = sqldbCheckAdmin -Instance $Instance -FindingDetails ([ref]$FindingDetails) -Status ([ref]$Status)
    #---=== End Custom Code ===---#
...
Function Get-V213905 {
    ...
    if ($res) {
        if ($res.role_name.count -eq 1 -and $res.role_member_name -eq 'dbo') {
            $FindingDetails = "The only account authorized to act as a db owner is 'dbo', but DISA still requires it be documented as authorized:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
        else {
            $FindingDetails = "DBA, Confirm that the following accounts are documented as authorized to act as database owners:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
    } # if ($res)
    ...
    if ($res) {
        if ($res.permission_name.count -eq 1) {
            $FindingDetails += "DBA, Confirm that the following account is documented as authorized to administer audits:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
        else {
            $FindingDetails += "DBA, Confirm that the following accounts are documented as authorized to administer audits:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
    } # if ($res)
...
Function Get-V213906 {
    ...
    "
    if ($res) {
        $FindingDetails += "DBA, ensure the following accounts are authorized in the server documentation to change procedures, functions and triggers:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213907 {
    ...
    "
    if ($res) {
        $FindingDetails += "DBA, ensure the following principals are authorized in the server documentation to own schemas:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
    else {
        $Status = "NotAFinding"
...
Function Get-V213908 {
    ...
    if ($res) {
        $FindingDetails += "DBA, Confirm that an approved server documentation documents the following accounts as authorized to own database objects:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213909 {
    ...
    if ($res) {
        ...
        # $Status = 'Open'
        $FindingDetails += "DBA, ensure the following accounts are authorized in the server documentation to modify objects:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213910 {
    ...
    if ($res) {
        $FindingDetails += "If the recovery model description does not match the documented recovery model, this is a finding.:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    } else {
    ...
    if ($res) {
        $FindingDetails += "Jobs set up to implement the backup plan:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    } else {
...
Function Get-V213911 {
    ...
    if ($res) {
        ...
        if ($res2) {
            $FindingDetails = "Review procedures and evidence of password requirements used to encrypt the following Database Master Keys:`n$($res2 | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
    }
    if ($FindingDetails -eq "") {
        $Status = "Not_Applicable"
...
Function Get-V213912 {
    ...
    if ($res) {
        $Status = 'Open'
        $FindingDetails += "DBA, ensure the server documentation has approved the encryption of these database master keys using the service master keys:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213914 {
    ...
    If ($DefaultDatabases.IndexOf($Database) -ne -1) {
        ...
        "
        if ($res) {
            $FindingDetails += "DBA, Review the database structure to determine where security related functionality is stored.`n If security-related database objects or code are not kept separate, this is a finding:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
        }
        $null = sqldbCheckAdmin -Instance $Instance -FindingDetails ([ref]$FindingDetails) -Status ([ref]$Status)
    }
    #---=== End Custom Code ===---#
...
Function Get-V213921 {
    ...
    if ($res) {
        #$Status = 'Open'
        $FindingDetails += "DBA, ensure the following accounts are authorized in the server documentation to own schemas:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213922 {
    ...
    "
    if ($res) {
        $FindingDetails += "DBA, ensure the following SQL modules are authorized in the server documentation to utilize impersonation:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213923 {
    ...
    if ($res) {
        #$Status = 'Open'
        $FindingDetails += "DBA, ensure the following principals are authorized in the server documentation to modify the specified object or type:`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
Function Get-V213924 {
   ...
   WHERE (D.database_id > 4)
    ...
    "
    if ($res) {
        $FindingDetails += "DBA, Remove unauthorized users from roles`n$($res | Format-Table | Out-String | Remove-TrailingSpaces)"
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAucGlI81S9FGZN
# 9wGGojfQ2eBo5ZAn+KWJmFvc2Kh61KCCHiQwggUqMIIEEqADAgECAgMTYdUwDQYJ
...
# BCDAIIvAkbzlmIfVD5UVvI3qm1Rs3R7Viu9vN4PkAP4u1DANBgkqhkiG9w0BAQEF
# AASCAQAxyX4lYRoYA+nL9NSAI1i1atQLOSyoOtbxX6TaQMFyTmqTKOZhhEtjNKKt
# 6SdZoaXQYmgx2wiHAJqypZByY7vZc852ta/T+Ny+UDs36An4WFvkinHO8VT1MBJY
# j30hwJ2enWFDYNQAXu3L1jnuljmdr8kjkmDUBVAXtsJzp83XOeG60Jbm3ZrCbHlj
# G2j7D4RUUHO/TWdEc36PL/avl9vrovd1LBvQWUuXE6YZgfiFeAPFUHewFuBnMtHO
# 3aXCNgoaABX6Wrrf9W9erqJtrWL0TJUlRs5w5/IgpQs46Lonmd81B61meQtyzZ4f
# l0hGM4rsC9PdHL0zl5t+dvaiSbk8oYIDJjCCAyIGCSqGSIb3DQEJBjGCAxMwggMP
# AgEBMH0waTELMAkGA1UEBhMCVVMxFzAVBgNVBAoTDkRpZ2lDZXJ0LCBJbmMuMUEw
# PwYDVQQDEzhEaWdpQ2VydCBUcnVzdGVkIEc0IFRpbWVTdGFtcGluZyBSU0E0MDk2
# IFNIQTI1NiAyMDI1IENBMQIQCoDvGEuN8QWC0cR2p5V0aDANBglghkgBZQMEAgEF
# AKBpMBgGCSqGSIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI1
# MTIxMTE4NTIxNVowLwYJKoZIhvcNAQkEMSIEILJ5aoX+ULNjO58N7hU8zhX1fjx6
# +O6PQSQrJORI7/D7MA0GCSqGSIb3DQEBAQUABIICABHDZcKfWD2y8f8G1Uu34kbo
# t4va68WKHoBhdzm8iftqYB7R37G3Sfm9kOjISzmx3skNYFneEiEXazxFJmxlHjg0
# Z/AMlblwqOSP5i69m0xvFDUJVgL6mrbDMqMbzdNt6dXZRLZR4OxVIdmhE+BF+Gdi
# 5qLZXqGLaCEPU1sSbxd9saVVahCPCU9LEpjbph66dPw5J2Z8ApGRBYGyBzd91jCb
# YUdU8gX/+w9gZmcumWZddnwOeat/xgHBCf+IsFaiLSq61nN6i2qYh4Q0Tzjukk94
# iKuC2SCfIwbaHXZv9tHbNG1huTwAO6WfqVNkuDdAMHjER64cfcLCHoQX08GR2qK1
# hwqQcjUc4HOhv5dZKWpTLD5tvwH47Kgy+xgwxAzhA6fGutxIU+KYMZ6qBjFnRNj4
# +u9FFhGX6b+vn4BucoTuB82Ok447PIy5Ns1ZyEvtnMEvWyBq4ryBcmj25rInH+z2
# DmfkI9Bn+HwURZatrAkUnR29M0RDBFy7Cd6vbsHHBCOVPrzGxYwAXIO+iG6z8EUh
# pOnx9lYpx9RFMYpE10b+tYB/Mqp7s9C1Ep16KQAL7spa83AT7s0waEUMmR3JfJzh
# EznLpwg42wpU1vChPPggS0ixM0f34s+Uwl9Rg3NnzEzHECWLqBkfZZtB4fVXXMzZ
# eIDNtpuVkJbMj+DqoPWd
